...
@section('content')
    ...
    <div class="container-fluid px-4">
        <div class="row justify-content-center">
            <div class="col-8">
                <div class="card mb-4">
                    <div class="card-header"><strong>Edit Absensi Laskar PLN</strong></div>
                    <div class="card-body">
                        @if ($errors->any())
                        ...
                        @endif
                        <form action="{{ route('attendances.update', $item->id)}}" method="POST" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <div class="form-group mb-3">
                                <label for="agenda">Agenda</label>
                                <input type="text" class="form-control form-control-sm @error('agenda') is-invalid @enderror" name="agenda" value="{{$item->agenda}}" id="agenda" placeholder="Masukan Agenda Rapat / Acara">
                                @error('agenda')
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="date" class="form-control form-control-sm @error('tgl_agenda') is-invalid @enderror" name="tgl_agenda" value="{{$item->tgl_agenda}}" id="tgl_agenda" placeholder="dd/mm/yyyy">
                                @error('tgl_agenda')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control form-control-sm @error('tempat') is-invalid @enderror" name="tempat" value="{{$item->tempat}}" id="tempat" placeholder="Masukan Tempat Acara">
                                @error('tempat')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="is_public" class="form-control form-control-sm @error('is_public') is-invalid @enderror" id="is_public" required>
                                    <option value="Ya" {{$item->is_public == 'Ya' ? 'selected' : ''}}>Ya</option>
                                    <option value="Tidak" {{$item->is_public == 'Tidak' ? 'selected' : ''}}>Tidak</option>
                                </select>
                                @error('is_public')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
