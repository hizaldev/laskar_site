        ...
        <div class="row justify-content-center">
            <div class="col-6">
                <div class="card mb-4">
                    ...
                    @endif
                    <form action="{{ route('dpc.store')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group mb-3">
                            ...
                            <select name="dpd_id" class="form-control form-control-sm @error('dpd') is-invalid @enderror" id="select2" required>
                                ...
                                @foreach ( $dpd as $d )
                                    <option value="{{$d->id}}">{{$d->dpd}}</option>
                                @endforeach
                            </select>
                            @error('dpd')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control form-control-sm @error('dpc') is-invalid @enderror" name="dpc" id="dpc" placeholder="masukan Nama DPC">
                            @error('dpc')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
